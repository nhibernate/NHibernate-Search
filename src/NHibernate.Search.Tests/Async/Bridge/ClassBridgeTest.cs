...
namespace NHibernate.Search.Tests.Bridge
{
    ...
    /// </summary>
    [TestFixture]
    public class ClassBridgeTestAsync : SearchTestCase
    {
        ...
        /// </summary>
        [Test]
        public async Task ClassBridgeAsync()
        {
            ISession s = this.OpenSession();
            ITransaction tx = s.BeginTransaction();
            await (s.SaveAsync(this.getDept1()));
            await (s.SaveAsync(this.getDept2()));
            await (s.SaveAsync(this.getDept3()));
            await (s.FlushAsync());
            ...
            // Cleanup
            foreach (object element in await (s.CreateQuery("from " + typeof(Department).FullName).ListAsync()))
            {
                await (s.DeleteAsync(element));
            }
            await (tx.CommitAsync());
            s.Close();
        }
        ...
        /// </summary>
        [Test]
        public async Task ClassBridgesAsync()
        {
            ISession s = this.OpenSession();
            ITransaction tx = s.BeginTransaction();
            await (s.SaveAsync(this.getDepts1()));
            await (s.SaveAsync(this.getDepts2()));
            await (s.SaveAsync(this.getDepts3()));
            await (s.SaveAsync(this.getDepts4()));
            await (s.FlushAsync());
            await (tx.CommitAsync());
            ...
            // Cleanup
            foreach (object element in await (s.CreateQuery("from " + typeof(Departments).FullName).ListAsync()))
            {
                await (s.DeleteAsync(element));
            }
            await (tx.CommitAsync());
        ...
        /// </summary>
        [Test, Explicit]
        public async Task ClassBridgesWithProjectionAsync()
        {
            ISession s = this.OpenSession();
            ITransaction tx = s.BeginTransaction();
            await (s.SaveAsync(this.getDepts1()));
            ...
            IEnumerator projections = (await (hibQuery.EnumerableAsync())).GetEnumerator();
